...
## Blueprint Supplement V3.1.2 — Naming Decisions (2026-03)
...
**AHS Hive:** Cell-based architecture IS a hive by definition.
Autonomous Cells coordinated without a central server.
The Queen does not control — she orchestrates.
Exactly what the shell does with Cells.
Biologically coherent: the hive functions even if individual cells die
(Azure Container Apps scale-to-zero).

**AHS Xinfer:** X = Excursion (FDA/GxP term for temperature deviation).
Pronounced "ex-in-fer". Connects with XaiDiagnostic, XaiRiskMonitor
already in codebase. Analysts and operators use "excursion" daily —
the name is self-explanatory in context.

### Rename Actions Required

```
AHS.Cell.ColdChain.* → AHS.Cell.Xinfer.*   (PM-RENAME-ColdChain-to-Xinfer.md)
AHS.Web.UI           → AHS.Web.Hive        (PM-RENAME-WebUI-to-Hive.md)
AHS.SaaS.slnx        → update both renames
```

### Biological Metaphor (canonical for documentation)

```
AHS Hive     = the organism
Cells        = autonomous organs
Xinfer       = the predictive nervous system
GxP Ledger   = immutable memory of the organism
AOT          = exists without external dependency (no JIT "oxygen")
