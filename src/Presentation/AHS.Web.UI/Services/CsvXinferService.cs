...
public class CsvXinferService : IXinferDemoService
{
    ...
    private async Task<IReadOnlyList<CsvShipmentRecord>> LoadDataAsync(CancellationToken ct)
    {
        ...
        var csvContent = await _httpClient.GetStringAsync("data/pharma/stress_test_40.csv", ct);
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            if (parts.Length >= 6)
            {
                ...
                var id = $"SHP-{Math.Abs(timestamp.GetHashCode()):X8}";
    ...
    {
        var records = await LoadDataAsync(ct);
        if (!records.Any())
        {
    ...
    public async Task<IReadOnlyList<AuditEvent>> GetAuditEventsAsync(CancellationToken ct = default)
    {
        ...
        for (int i = 0; i < Math.Min(records.Count, 20); i++)
        {
            ...
            var hashBytes = sha.ComputeHash(Encoding.UTF8.GetBytes(r.Id + r.Status + timestamp.Ticks));
